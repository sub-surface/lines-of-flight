...
- **No diegetic chrome inside the world.** The only marks *in the canvas* are: the Dot, the beings, your ink, and the precipitated voice. White is not a background — it is the world's substance.
  ...
  - **The boot veil** *(implemented)*: a full-screen paper-white landing that fades three serif lines in, invites you (*"— move, or touch, to begin —"*), then dissolves. It is **not a menu and not a loading gate**: the sim is already running underneath (field baking, glyph cache warming, ambient events spawning weather), so the veil is just a held breath that lets the world arrive alive rather than blank. Any input lifts it; a ~6s max-dwell lifts it for an idle visitor so no one is ever held. Honours `prefers-reduced-motion` (no staged fades) and is skipped entirely under `?bench`. After fading it removes itself from the DOM so it never intercepts a later event.
...
- **Performance** *(implemented — these were the real wins):*
  - **Baked field grid.** The curl-noise + per-source math is evaluated onto a coarse `~36px` grid (`bakeField`); particles read it by **bilinear interpolation** — O(1), no trig, no allocation per particle. This was the dominant per-frame cost and has since been driven ~7.6× lower (≈4.6ms → ≈0.6ms) by three compounding wins: (1) the curl is taken **analytically** rather than by 4-sample finite difference (exact, ~3× fewer trig); (2) the bake is **separable** — the curl's x- and y-dependent trig take only `FCOLS`/`FROWS` distinct values, computed once per axis per frame, so ~3600 trig/frame collapse to ~180; (3) it is **temporally gated** — a fully static world (no camera motion, crossfade, living source, or event) re-bakes only every other frame, since the field's idle drift is imperceptible. Interpolating a baked field is also *more accurate*: smoother, coherent flow versus independent per-particle noise.
  - **Age-staggered field sampling.** Reading the baked grid is the ink loop's dominant cost, so its refresh rate scales with particle age: young/text ink every step, mid-age every other, old drifting ink every fourth — reusing a cached vector between. A several-step-stale weather vector is invisible on fading, barely-moving ink, and old ink is the majority.
...
- **The field.** `evalField(x,y)` returns a velocity: a divergence-free **curl-noise** base (the breath of smooth space) plus a sum of **sources**, each warping space via a unified **complex potential flow formula** — `vx += (A·dx + B·dy) / (d² + R²)`, `vy += (A·dy − B·dx) / (d² + R²)`. The coefficients `A` (radial) and `B` (rotational) encode the smooth source types (attract, repel, swirl, spiral) without branching or square roots. Beyond that, named primitives add the shapes potential flow can't make: `lane` (striated channels), `cell` (gyre lattice / a true grid), `attractor` (de Jong strange-attractor flow — chaos as a field), `rankine` (cored vortex), `fbm` (multi-octave turbulence), `shiver` (noise). All compose additively; they are faces of one field, not separate mechanics, and all are evaluated only on the baked grid.
...
- **Beats.** The live source-set is a **story state** — now **25 of them**, spanning calm wells/vortices, the assemblage pairs (`two`, `dyad`, `chain`), ruled `striated`/`grid`/`lattice` space, strange `attractor` variants, and real fluid flows (`whirlpool`, `vortex-street`, `double-gyre`, `turbulence`, `spiral`, `channel`, `fold`). Click/→ advance, ← back; each speaks a Deleuze line on entry, and entering a beat **eased-crossfades** the field (~2.5s) rather than snapping (per-source `bl` blend in `stepBeat`). The plateaus after the opening `still` are **shuffled each load** — the arc is the player's to interpret, not authored.
...
- **Voice.** ~100 aphorisms from Deleuze, Guattari, Spinoza, Bergson, Blanchot — Fisher-Yates shuffled on load, cycling and reshuffling so the world never goes silent. Text precipitates as **ink particles held by a spring** (glyph points shuffled so the line condenses rather than wipes in), graduates smoothly into the field flow (txt `1.0→0` across the `TEXT.holdHi`/`holdSpan` window), and on full release receives a field-velocity kick so it merges with the surrounding weather rather than sitting inert.

## 8. Build order

1. **Proof of soul** *(done)* — `index.html`: the carried Dot, field-advected ink that diffuses into weather, **25 eased-crossfading field-states** (wells, vortices, lanes, grids, strange attractors, real fluid flows) shuffled each load, ambient events, and `difference`-blended serif text that fades in/out of negative space. Movement I's opening, in miniature.
...
- **`txt` decay window.** `txt = clamp((TEXT.holdHi − f) / TEXT.holdSpan, 0, 1)` — linearly interpolates from held to free across the configured window. All text-lifecycle constants live in one `TEXT` object.
...
- **2× rasterisation scale.** Glyph canvas rendered at 2× font size (`willReadFrequently`, so `getImageData` stays cheap), sampled at 2-pixel step, coordinates divided back by 2. Sub-pixel fidelity without inflating particle counts. Canvas height `2.0×` font size prevents ascender/descender clipping. Scanned glyph points are **shuffled** so a line precipitates (condenses everywhere at once) rather than wiping in left-to-right.
- **Deferred spawn queue.** `speak()` stores `g.pts` in a queue; `drainSpawnQueue()` births particles across ~10 frames. Eliminates the `getImageData` + bulk-spawn spike that previously stalled a frame by 30–50ms. Beat voices are pre-warmed one per frame behind the boot veil rather than in a startup burst. On-screen line life scales with line length.
...
The curl-noise base uses summed sines with fixed spatial frequencies (`0.0042`, `0.0051`, etc.). As the canvas grows large or the game runs long, the *base* breathing can feel repetitive. **Partly addressed:** the `turbulence` beat now adds a second analytic-curl octave (fractional Brownian motion), giving a geological, eddies-within-eddies weather; and the strange-`attractor` beats give found-not-generated structure. **Remaining direction:** fold an extra octave into the *base* itself (cheap now that the bake is separable), or a slowly-rotating gradient table, so even `still` feels less periodic over long sessions.
